...
//    std::cout<<"num_of_points_to_flip is "<<(ss->num_of_points_to_flip)<<std::endl;
    ...
    for(int j=0; j<length(neighbors); j++){
		  if(index(neighbors,j)!=spin1) break; //not inside a grain, break from for int j
      else number_of_same_neighours++;
    }
    if(number_of_same_neighours==length(neighbors)){//inside a grain
//      hh--; // DELETE THIS LINE UNLESS IN DEBUG TEST!!!
...
//        double temperature=((ss->temperature_along_x))[x[0]];
			  for (int i=-1; i<=1; i++){
				  for (int j=-1; j<=1; j++){
					  ...
					  r[1] = x[1] + j;
					  int spin = (*(ss->grid))(r);  
//            dE += 1.0/2*((spin!=spin2)-(spin!=spin1))*LargeAngleGrainBoundaryEnergy(temperature); // grain boundary energy  
            dE += 1.0/2*((spin!=spin2)-(spin!=spin1)); // grain boundary energy  
				  }
        }
      }
      if(dim==3){
...
//      kT = 1.3806488e-23*((ss->temperature_along_x))[x[0]];
        kT = 1.3806488e-23*573.0;
			if (dE<=0.0){
        int rank = MPI::COMM_WORLD.Get_rank();
        if(rank==0) std::cout<<"before, at "<<x[0]<<" "<<x[1]<<" "<<x[2]<<" "<<(*(ss->grid))(x)<<"  ";
        (*(ss->grid))(x) = spin2;
        if(rank==0) std::cout<<"after, at "<<x[0]<<" "<<x[1]<<" "<<x[2]<<" "<<(*(ss->grid))(x)<<"\n";
//	int rank = MPI::COMM_WORLD.Get_rank();
//      if(rank==0 && dE<=0.0) std::cout<<"dE is "<<dE<<std::endl;
      }
			else if(r<exp(-dE/kT)){
        (*(ss->grid))(x) = spin2;
      }
