...
/*
 ...
 //                 std::cout<<"cosine_nd_trione is "<<cosine_nd_trione<<" "<<h1<<" "<<k1<<" "<<l1<<std::endl;
                ...
                if(i==0){//i=0 j!=0   x[0] along rolling direction
                  ...
                  if(cosine_td_trione>cos_tolerance){
                    ...
                    if(cosine_td_trione>cos_tolerance){
                      ...
                      dE += -1.0/2*CoherentTwinBoundaryEnergy(temperature);
                    }
                  }
              ...
              if(incoherent_twin_boundary == true){
                dE += -1.0/2*IncoherentTwinBoundaryEnergy(temperature);
              }
              else if(incoherent_twin_boundary == false && coherent_twin_boundary == false){
                dE += -1.0/2*(spin!=spin1)*LargeAngleGrainBoundaryEnergy(temperature); // grain boundary energy  
             } 
              ...
              if(incoherent_twin_boundary == true){
                ...
                dE += 1.0/2*(spin!=spin2)*LargeAngleGrainBoundaryEnergy(temperature); // grain boundary energy  
              }*/
//            }
				  }
        }
	...
	int rank = MPI::COMM_WORLD.Get_rank();
      if(rank==0 && dE<=0.0) std::cout<<"dE is "<<dE<<"  exp(-dE/kT) is  "<<exp(-dE/kT)<<std::endl;
